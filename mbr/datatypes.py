...
class direction(object):
    ...
    compass_dirs_ = {'N': _('NORTHWARD'), 'E': _('ESTERLY'), 'S': _('SOUTHERLY'), 'W': _('WESTERLY'),
                     'NE': _('NORTHEASTERLY'), 'SE': _('SOUTHEASTERLY'), 'SW': _('SOUTHWESTERLY'),
                     'NW': _('NORTHWESTERLY'), 'NDV': _('NDV')}

    def __init__(self, d, compass=None):
        if d in direction.compass_dirs:
            self._compass = d
            self._degrees = direction.compass_dirs[d]
            self._direct = direction.compass_dirs_[d]
        else:
            self._compass = None
            self._direct = None
            value = float(d)
    ...
    def compass_str(self):
        if self._compass in self.compass_dirs_:
            return self.compass_dirs_[self._compass]
        else:
